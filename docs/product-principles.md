...
- **访客 / 读者**：消费公开表达（文章、笔记、项目、专题、搜索等）；可注册、登录时仍**不**获得创作能力，除非被提升为管理员（以实际 RBAC 与账号策略为准）。
...
| 面 | 职责 |
| --- | --- |
| **Public Web** | 对外认知与阅读：首页、文章 / 笔记 / 项目 / 搜索入口、评论与联系等。 |
...
- **项目**：结构化承载背景、目标、技术、结果，并**关联**文章、笔记和附件。
- **附件**：丰富表达维度与证据链，作为文章、笔记和项目的证明材料。
...
3. **关系优先、不写死层级**：项目—文章—笔记等用**关系**表达，避免目录式硬编码。
...
- **高优先级**：公开展示、**管理员**在 Studio 中的内容管理与发布、身份与登录、评论、搜索、项目与笔记、AI 摘要 / 草稿 + 审阅路径、权限不串线（非管理员不得获得创作入口）。
