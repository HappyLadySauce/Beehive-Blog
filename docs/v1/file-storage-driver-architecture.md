...
当前附件系统以 `attachment.attachments` 为核心，直接在附件行中保存 `storage_type`、`bucket`、`object_key`、`local_path` 等字段。这种设计能覆盖 `local`、`s3`、`oss` 的早期场景，但有两个明显限制：
...
- 旧附件数据可分阶段迁移，不一次性破坏现有上传、下载和头像引用。
...
2. **文件绑定存储实例，不再只绑定存储类型**
   新文件必须写入 `storage_mount_id`。`storage_type` 只作为旧数据兼容字段或响应中的派生展示字段。
...
5. **迁移优先兼容，废弃字段分阶段移除**
   先新增表和字段，再回填默认 mount，最后调整写路径；确认稳定后再删除旧字段。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `storage_mount_id` | `BIGINT NULL` | 指向 `attachment.storage_mounts(id)` |
...
- `bucket`、`local_path`、`storage_type` 进入兼容期；新写路径不再依赖这些字段做驱动选择。
- 远端下载、直传、删除统一通过 `storage_mount_id -> driver_name -> DriverBackend` 解析。

兼容策略：

- 旧 `local` 附件回填到默认 local mount，`object_key` 可由 `local_path` 回填。
- 旧 `s3` / `oss` 附件分别回填到默认 s3 / oss mount。
- 兼容期响应可继续输出 `storage_type`、`bucket`、`local_path`，但新增响应应包含 `storage_mount_id`。
...
- 根据现有配置创建默认 storage mount。

### 阶段二：回填旧数据

- `storage_type = local`：回填到 local mount，`object_key` 优先使用已有 `object_key`，否则由 `local_path` 回填。
- `storage_type = s3`：回填到 s3 mount，保留 `bucket` 到 mount config 或兼容字段。
- `storage_type = oss`：回填到 oss mount，保留 `bucket` 到 mount config 或兼容字段。
- 对无法回填的行保留旧逻辑读取，并输出迁移告警日志。

### 阶段三：切换写路径

- 上传和预签名接口接受 `storage_mount_id`。
- 未传 `storage_mount_id` 时使用默认 mount。
- 新附件必须写入 `storage_mount_id`。
- 新代码不再根据 `storage_type` 选择驱动。

### 阶段四：收敛旧字段

- 确认所有活跃附件均有 `storage_mount_id`。
- 将 `storage_type` 改为响应派生字段或废弃字段。
- 后续独立迁移可删除 `bucket`、`local_path` 等旧定位字段。
