...
export async function GET() {
  try {
    const result = await forwardSiteRequest<SettingsResponse>({ method: "GET" });
...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await forwardSiteRequest<SettingsResponse>({ method: "PATCH", body: JSON.stringify(body) });
    return settingsResponse(result);
  } catch (error) {
    return jsonError(error);
  }
...
async function forwardSiteRequest<T extends SettingsResponse>(init: RequestInit): Promise<SettingsForwardResult> {
  const cookieStore = await cookies();
  const accessToken = cookieStore.get(accessCookieName)?.value;
  const refreshToken = cookieStore.get(refreshCookieName)?.value;
  if (accessToken) {
    try {
      return { data: await forwardGoApi<T>("/settings/site", withBearer(init, accessToken)) };
    } catch (error) {
  ...
  const data = await forwardGoApi<T>("/settings/site", withBearer(init, refreshedAuth.token.access_token));
