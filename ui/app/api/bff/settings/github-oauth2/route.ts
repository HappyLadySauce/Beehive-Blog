...
const settingsGithubOAuth2Path = "/settings/github-oauth2";
...
async function forwardSettingsRequest<T extends SettingsResponse>(init: RequestInit): Promise<SettingsForwardResult> {
  ...
  if (accessToken) {
    try {
      const data = await forwardGoApi<T>(settingsGithubOAuth2Path, withBearer(init, accessToken));
      return { data };
    } catch (error) {
  ...
  const data = await forwardGoApi<T>(settingsGithubOAuth2Path, withBearer(init, refreshedAuth.token.access_token));
