...
const settingsAttachmentPath = "/settings/attachment";
...
async function forwardSettingsRequest<T extends SettingsResponse>(init: RequestInit): Promise<SettingsForwardResult> {
  ...
  if (accessToken) {
    try {
      const data = await forwardGoApi<T>(settingsAttachmentPath, withBearer(init, accessToken));
      return { data };
    } catch (error) {
  ...
  const data = await forwardGoApi<T>(settingsAttachmentPath, withBearer(init, refreshedAuth.token.access_token));
