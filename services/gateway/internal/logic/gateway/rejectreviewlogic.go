...
func (l *RejectReviewLogic) RejectReview(req *types.ReviewDecisionRequest) (resp *types.ReviewTask, err error) {
	if req == nil || req.Id <= 0 {
	...
	startedAt := time.Now()
	defer func() {
		auditRPC(l.ctx, "content.reject_review", "content.RejectReview", startedAt, err, fields)
	}()

	out, err := l.svcCtx.Content.RejectReview(l.ctx, &contentrpc.RejectReviewRequest{
		Id:             req.Id,
		Reason:         req.Reason,
