...
- 附件系统：文件存储通过 `attachment.storage_drivers`、`attachment.storage_mounts` 和 `attachment.attachments.storage_mount_id + object_key` 定位，驱动配置来自数据库 mount；`attachment.categories` 使用物化路径树
